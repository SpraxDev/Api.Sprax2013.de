...
let profileSeenNamesPersister: ProfileSeenNamePersister;
...
describe('#persist', () => {
  ...
  test.each([null, today])('Persisting a never-seen name (seenAt=%j)', async (seenAt: Date | null) => {
    ...
    expect(databaseTransaction.profileSeenName.upsert).toHaveBeenCalledTimes(1);
    expect(databaseTransaction.profileSeenName.upsert).toHaveBeenCalledWith(expect.objectContaining({
