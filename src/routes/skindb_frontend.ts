...
router.all('/skin/:skinID/vote', (req, res, next) => {
  restful(req, res, {
    post: () => {
      ...
      if (typeof req.body.vote != 'boolean' && req.body.vote != 'unset') {
        invalidBody.push({
          param: 'vote',
          condition: `Boolean value or 'unset'`
        });
      }
...
router.all('/skin/:skinID', (req, res, next) => {
  restful(req, res, {
    get: () => {
      ...
      if (req.query.profile && !isUUID(req.query.profile as string)) {
  ...
  restful(req, res, {
    get: async () => {
      ...
      let directProfileHit: SkinDBSearch['profiles']['direct'] = null,
          ...
          skinHits: { skins: Skin[], time: number, moreAvailable: boolean } = {
            skins: [],
            time: 0,
            moreAvailable: false
          };
      ...
      if (req.header('Content-Type') == 'image/png') {
        ...
        Image.fromImg(req.body, (err, queryImg) => {
          if (err || !queryImg) {
            return next(err || new ErrorBuilder().invalidBody([{
              param: 'body',
              condition: 'Valid png (max. 3MB)'
            }]));
          }

          if (!queryImg.hasSkinDimensions()) {
            return next(new ErrorBuilder().invalidBody([{
              param: 'body',
              condition: 'Valid skin file (wrong image dimensions)'
            }]));
          }
        ...
        if (query.length <= 16) {
          try {
            indirectProfileHits = (await db.searchProfile(query, 4, 0))
                .map((elem) => {
                  return {name: elem.profile.name, matched_name: elem.matched_name, id: elem.profile.id};
                })
